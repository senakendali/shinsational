...
export function render(target, params, query = {}, labelOverride = null) {
  ...
  .then(async ([headerMod, footerMod, serviceMod, loaderMod, toastMod]) => {
    ...
    try {
      const cache = JSON.parse(localStorage.getItem('kol_profile') || 'null');
      if (cache) {
        fillForm(cache);
      }
    } catch {}
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (me?.tiktok_user_id && !tiktokIdEl.value) tiktokIdEl.value = me.tiktok_user_id;
        if (me?.tiktok_full_name && !fullNameEl.value) fullNameEl.value = me.tiktok_full_name;
        if (me?.tiktok_username && !usernameEl.value) usernameEl.value = normalizeHandle(me.tiktok_username);
    ...
    async function onHandleChange() {
      ...
      try {
        const q = new URLSearchParams({ tiktok_user_id: tiktokIdEl.value });
