// /js/pages/privacy/index.js  (atau path file kamu)
export function render(target, params, query = {}, labelOverride = null) {
  const v = window.BUILD_VERSION || Date.now();
  console.log('[privacy/index render] v=', v);

  // Load header + footer TANPA await (router kamu gak perlu async)
  Promise.all([
    import(`/js/components/headerKol.js?v=${v}`),
    import(`/js/components/footerKol.js?v=${v}`)
  ])
    .then(([h, f]) => {
      const { renderHeaderKol } = h;
      const { renderFooterKol } = f;
      renderHeaderKol('header');
      renderFooterKol();
    })
    .catch(err => console.error('[Import header/footer failed]', err));
      ...
      <div class="container pt-5">
        ...
        <div class="mb-4">
          ...
          <ul>
            ...
            <li>Memberikan layanan pelacakan konten sesuai kebutuhan pengguna.</li>
