...
export async function render(target, path, query = {}, labelOverride = null) {
    ...
    const [
        ...
        import(`../../components/popUpImage.js?v=${v}`),
    ]);
    ...
    const { showImageModal } = popUpImageMod;
  ...
  `;
    ...
    const fmtNum = (n) =>
        n === 0 || n ? Number(n).toLocaleString("id-ID") : "-";
        ...
        for (let i = 1; i <= 5; i++) {
            const hasAny = subs.some(
                (s) =>
                    s[`link_${i}`] ||
                    s[`post_date_${i}`] ||
                    s[`screenshot_${i}_url`] ||
                    s[`screenshot_${i}_path`] ||
                    s[`invoice_${i}_url`] ||
                    s[`invoice_${i}_path`] ||
                    s[`review_proof_${i}_url`] ||
                    s[`review_proof_${i}_path`] ||
                    s[`review_${i}_url`] ||
                    s[`review_${i}_path`] ||
    ...
    async function loadSubmissions(page = 1) {
        ...
        try {
          ...
          <div class="table-responsive">
            ...
            /// Button to open image in modal or to url///
            const btn = (u, label = "View") => {
                if (!u) return '<span class="text-muted">-</span>';

                if (u.match(/\.(jpe?g|gif|png)$/i)) {
                    // Create a unique ID for each button
                    const btnId =
                        "imgBtn_" + Math.random().toString(36).substr(2, 9);

                    // Store the URL in a data attribute
                    return `<a href="#" id="${btnId}" 
                  class="btn btn-sm btn-outline-secondary view-image-btn" 
                  data-image-url="${u.replace(/"/g, "&quot;")}">
                  ${label}
               </a>`;
                }
                // Regular link for non-image URLs
            ...
            for (let i = 1; i <= maxSlots; i++) {
                theadTopGroups.push(
                    `<th colspan="${SLOT_COLS}" class="text-center borderHeader ${
                        i % 2 === 0 ? "bg-light" : ""
                    }">Content ${i}</th>`
                );
            }
            ...
            for (let i = 1; i <= maxSlots; i++) {
                theadSub.push(
                    ...subMetrics.map(
                        (m) =>
                            `<th class="text-center borderHeader ${
                                i % 2 === 0 ? "bg-light" : ""
                            }" style="min-width:120px">${m}</th>`
                    )
                );
            }
            ...
            for (const s of filtered) {
                ...
                const hasResi = !!(
                    s.shipping_tracking_number || s.shipping_courier
                );
                ...
                data-courier="${
                    s.shipping_courier
                        ? String(s.shipping_courier).replace(/"/g, "&quot;")
                        : ""
                }"
                data-resi="${
                    s.shipping_tracking_number
                        ? String(s.shipping_tracking_number).replace(
                              /"/g,
                              "&quot;"
                          )
                        : ""
              ...
              <i class="bi bi-truck"></i> ${hasResi ? "Ubah Resi" : "Isi Resi"}
                ...
                for (let slot = 1; slot <= maxSlots; slot++) {
                    ...
                    const invPath =
                        s[`invoice_${slot}_url`] ||
                        s[`invoice_${slot}_path`] ||
                        s.invoice_file_url ||
                        s.invoice_file_path;
                    ...
                    const revPath =
                        ...
                        s.review_proof_file_url ||
                        s.review_proof_file_path;
                    const revUrl = toFileUrl(revPath);

                    const views =
                        metric(s, slot, "views") ?? metric(s, slot, "view");
                    const likes =
                        metric(s, slot, "likes") ?? metric(s, slot, "like");
                    const comments =
                        metric(s, slot, "comments") ??
                        metric(s, slot, "comment");
                    const shares =
                        metric(s, slot, "shares") ?? metric(s, slot, "share");

                    const evenBg = slot % 2 === 0 ? "bg-light" : "";

                    slotCells.push(
                        `<td class="text-center ${evenBg}">${
                            link
                        ...
                        `<td class="text-center ${evenBg}">${
                            likes != null ? fmtNum(likes) : "-"
                        }</td>`,
                        `<td class="text-center ${evenBg}">${
                            comments != null ? fmtNum(comments) : "-"
                        }</td>`,
                        `<td class="text-center ${evenBg}">${
                            shares != null ? fmtNum(shares) : "-"
                        }</td>`
                    );
                }
          ...
          <tr data-submission-id="${s.id}">
            <!-- sticky left -->
            <td class="sticky-col sticky-left border-bottom pb-2">
              <div class="d-flex align-items-center gap-2">
                ${
                    avatarUrl
                        ? `<img src="${avatarUrl}" alt="" style="width:34px;height:34px;border-radius:50%;object-fit:cover">`
                ...
                <div>
                  ...
                  ${
                      addr
                          ? `<div class="text-muted small text-truncate" style="max-width:180px" title="${addr}">${addr}</div>`
                          : ""
                  }
                </div>
              </div>
            ...
            <!-- sticky right -->
            <td class="sticky-col sticky-right border-bottom pb-2">
        ...
        <div class="submissions-wrapper">
          <div class="submissions-scroll">
            <table class="table table-bordered submissions-table">
              <thead>
                <tr>
                  <th class="text-center sticky-col sticky-left borderHeader">KOL Name</th>
                  ${theadTopGroups.join("")}
                  <th class="text-center sticky-col sticky-right borderHeader">Actions</th>
                </tr>
                <tr>
    ...
    function exportVisibleToCSV() {
        ...
        for (const s of lastFiltered) {
            ...
            for (let slot = 1; slot <= 5; slot++) {
                ...
                const pdate = s[`post_date_${slot}`] || "";
                const scPath =
                    s[`screenshot_${slot}_url`] || s[`screenshot_${slot}_path`];
                const scUrl = toFileUrl(scPath) || "";
                const invUrl =
                    toFileUrl(
                        s[`invoice_${slot}_url`] ||
                ...
                const likes =
                    metric(s, slot, "likes") ?? metric(s, slot, "like");
                const comments =
                    metric(s, slot, "comments") ?? metric(s, slot, "comment");
                const shares =
                    metric(s, slot, "shares") ?? metric(s, slot, "share");
        ...
        const cleanName = (lastCampaignName || "submissions").replace(
            /[^\w\-]+/g,
            "_"
        );
    ...
    // ===== Modal helpers (Image) click handler
    ...
    resiBtnSave?.addEventListener("click", async () => {
        ...
        if (!id) {
            showToast("ID submission tidak valid.", "error");
            return;
        }
        if (!courier || !tracking) {
            showToast("Nama ekspedisi dan nomor resi wajib diisi.", "error");
            return;
        }
    ...
    function attachActionHandlers() {
            ...
            btn.addEventListener("click", async (e) => {
                ...
                try {
                    ...
                    if (
                        ...
                        showToast(
                            err?.message ||
                                "Token TikTok tidak valid. Silakan connect ulang.",
                            "error"
                        );
                    } else {
                        showToast(
                            err?.message || "Gagal refresh metrik",
                            "error"
                        );
                    }
                } finally {
        ...
        if (exportBtn) {
            exportBtn.onclick = () => {
                ...
                const url = `/api/influencer-submissions/export?campaign_id=${encodeURIComponent(
                    campaignId
