...
export async function render(target, path, query = {}, labelOverride = null) {
  ...
  // ===== STATE: jumlah konten wajib per KOL (default 1) =====
  let requiredSlots = 1;
  ...
  // ==== Ambil "konten wajib per KOL" dari campaign (PRIORITAS: content_quota) / KPI (maksimal 5 kolom)
  async function getCampaignContentPerKol(campaignId) {
    ...
    try {
      ...
      const c = detail?.data ?? detail?.campaign ?? detail ?? null;
      if (!c) return 1;

      // 1) PRIORITY: content_quota di campaign
      const quotaKeys = [
        "content_quota", "contents_quota", "contentQuota", "quota_contents",
        "quota_per_kol", "content_quota_per_kol"
      ];
      ...
      // 2) Fallback: *per_kol fields yang lama
      const direct = Number(
        c.contents_per_kol ?? c.content_per_kol ?? c.posts_per_kol ?? c.post_per_kol
      );
      if (Number.isFinite(direct) && direct > 0) return Math.min(5, direct);

      // 3) Fallback: kpi_targets.*
      const raw = c.kpi_targets ?? c.kpi ?? c.kpiTargets ?? null;
      let kpi = null;
      if (typeof raw === "string") { try { kpi = JSON.parse(raw); } catch {} }
      else if (raw && typeof raw === "object") { kpi = raw; }
  ...
  try {
    ...
    // inisialisasi requiredSlots dari campaign terpilih (kalau ada)
    requiredSlots = await getCampaignContentPerKol(campaignFilter.value || "");
  } catch {}
  ...
  async function loadSubmissions(page = 1) {
    ...
    try {
      ...
      const btn = (u, label = "View") => {
        ...
        if (u.match(/\.(jpe?g|gif|png|webp|bmp)$/i)) {
          const btnId = "imgBtn_" + Math.random().toString(36).substr(2, 9);
      ...
      for (let i = 1; i <= maxSlots; i++) {
        theadSub.push(
          ...subMetrics.map(
            (m) =>
              `<th class="text-center borderHeader ${i % 2 === 0 ? "bg-light" : ""}" style="min-width:120px">${m}</th>`
          )
        );
      }
      ...
      for (const s of filtered) {
        ...
        for (let slot = 1; slot <= maxSlots; slot++) {
          ...
          slotCells.push(
            `<td class="text-center ${evenBg}">${link ? btn(link, "View Content") : '<span class="text-muted">-</span>'}</td>`,
      ...
      const tableHtml = `
        <style>
          ...
          thead th.sticky-left, thead th.sticky-right { z-index: 7; }
      ...
      // Pagination
      pager.innerHTML = "";
      const lastPage = res?.last_page ?? res?.meta?.last_page ?? res?.pagination?.last_page ?? 1;
      const curPage = res?.current_page ?? res?.meta?.current_page ?? res?.pagination?.current_page ?? page;
      if (lastPage > 1) {
  ...
  function exportVisibleToCSV() {
    ...
    const maxExportSlots = Math.max(1, Number(requiredSlots) || 1); // ikut campaign (content_quota)
    ...
    for (const s of lastFiltered) {
      ...
      for (let slot = 1; slot <= maxExportSlots; slot++) {
        const link = s[`link_${slot}`] || "";
        const pdate = s[`post_date_${slot}`] || "";
        const scPath = s[`screenshot_${slot}_url`] || s[`screenshot_${slot}_path`];
        const scUrl = toFileUrl(scPath) || "";
  ...
  campaignFilter.addEventListener("change", async () => {
    ...
    currentPage = 1;
    // update jumlah kolom konten sesuai content_quota campaign
  ...
  if (campaignFilter.value) {
    currentCampaignId = campaignFilter.value;
    // pastikan requiredSlots sesuai content_quota sebelum load
    requiredSlots = await getCampaignContentPerKol(currentCampaignId);
    loadSubmissions(currentPage);
  } else {
    hideLoader();
